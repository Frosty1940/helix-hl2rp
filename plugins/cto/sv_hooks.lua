						...
						elseif (#data[client] < 1) then
							...
							if (#violations > 0) then
								if (!client:IsCombine() and !client:GetNetVar("IsBiosignalGone")) then
									...
									Schema:AddCombineDisplayMessage("@ctoMovementViolation", Color(255, 128, 0, 255), combineCamera:EntIndex())
								end
							end
						end
					end
	...
	if (client:IsCombine()) then
		...
		if (client:GetNetVar("IsBiosignalGone")) then
			if (ix.config.Get("useBiosignalSystem")) then
				client:NotifyLocalized("ctoNoBioSignal")
			else
...
function PLUGIN:OnCharacterFallover(client, entity, bFallenOver)
	if (client:IsCombine() and !client:GetNetVar("IsBiosignalGone")) then
		if (bFallenOver) then
			...
			Schema:AddCombineDisplayMessage("@ctoTraumaPacket", Color(255, 255, 255, 255))
			Schema:AddCombineDisplayMessage("@ctoLostConsciousness", Color(255, 0, 0, 255), digits, location)
		end
	end
end
