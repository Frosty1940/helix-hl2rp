...
function PLUGIN:DoPostBiosignalLoss(client)
	...
	Schema:AddCombineDisplayMessage("@ctoLostBiosignalDownload", Color(255, 255, 255, 255))
	Schema:AddCombineDisplayMessage("@ctoLostBiosignal", Color(255, 0, 0, 255), digits, location)
	...
	if (client:IsCombine()) then
		...
		elseif (!bEnable and isDisabledAlready) then
			...
			if (bEnable) then
				...
				client:AddCombineDisplayMessage("@ctoConnectionRestored", Color(0, 255, 0, 255)) -- Alert this unit.

				local digits = string.match(client:Name(), "%d%d%d%d?%d?") or 0

				-- Alert all units.
				Schema:AddCombineDisplayMessage("@ctoFoundBiosignalDownload", Color(255, 255, 255, 255))
				Schema:AddCombineDisplayMessage("@ctoNoncohesiveBiosignal", Color(0, 255, 0, 255), digits, location)
				...
				client:AddCombineDisplayMessage("@ctoShuttingDown", Color(255, 0, 0, 255)) -- Alert this unit.
...
function PLUGIN:DispatchRequestSignal(client, text)
	...
	Schema:AddCombineDisplayMessage("@ctoAssistanceRequest", Color(175, 125, 100, 255))
end
