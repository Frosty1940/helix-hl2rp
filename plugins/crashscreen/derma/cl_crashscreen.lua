...
function PANEL:PaintOver()
	draw.DrawText(L"connectionLost", "ixTitleFont", ScrW() / 2, 150, color_white, TEXT_ALIGN_CENTER)

	if self.ServerIsOff == nil then
		draw.DrawText(L"checkingServer", "ixBigFont", ScrW() / 2, ScrH() / 1.5, color_white, TEXT_ALIGN_CENTER)
		return
	end

	if self.ServerIsOff then
		draw.DrawText(L"serverOffline", "ixBigFont", ScrW() / 2, ScrH() / 1.5, color_white, TEXT_ALIGN_CENTER)
	else
		draw.DrawText(L"lostConnection", "ixBigFont", ScrW() / 2, ScrH() / 1.5, color_white, TEXT_ALIGN_CENTER)
		draw.DrawText(L"checkInternet", "ixBigFont", ScrW() / 2, h + 160, color_white, TEXT_ALIGN_CENTER)
	end
end
