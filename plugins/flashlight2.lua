...
PLUGIN.description = "This plugin makes your flashlight feels like it's from L4D2."


IX_CVAR_FLASHLIGHT_EFFECT = CreateClientConVar("ix_flash_effects", 0, true, true)
IX_CVAR_FLASHLIGHT_OTHER = CreateClientConVar("ix_flash_other", 1, true, true)

ix.lang.AddTable("english", {
	flashlight = "Flashlight",
	optToggleFlashEffects = "Toggle Flashlight Effects",
	optToggleFlashStuffs = "Toggle Other Person's Flashlight",
})
ix.lang.AddTable("korean", {
	flashlight = "손전등",
	optToggleFlashEffects = "손전등 효과 켜기",
	optToggleFlashStuffs = "다른 사람 손전등 보기",
})

if (CLIENT) then
	ix.option.Add("toggleFlashEffects", ix.type.bool, false, {
		category = "flashlight",
		OnChanged = function(oldValue, value)
			if (value == true) then
				RunConsoleCommand("ix_flash_effects", "1")
			else
				RunConsoleCommand("ix_flash_effects", "0")
			end
		end
	})

	ix.option.Add("toggleFlashStuffs", ix.type.bool, false, {
		category = "flashlight",
		OnChanged = function(oldValue, value)
			if (value == true) then
				RunConsoleCommand("ix_flash_other", "1")
			else
				RunConsoleCommand("ix_flash_other", "0")
			end
		end
	})
end
	...
	local meta = FindMetaTable("Player")
	 ...
	 if (CLIENT) then
		...
		local function flashlightThink(client)
			...
			if (!client:GetNoDraw() and IX_CVAR_FLASHLIGHT_EFFECT:GetBool() != false) then
				if (client:GetNWBool("customFlashlight") == true) then
