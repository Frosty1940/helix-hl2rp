...
function PLUGIN:EntityTakeDamage(entity, dmgInfo)
	if (entity:GetClass() == "prop_door_rotating" and (entity.ixNextBreach or 0) < CurTime()) then
		...
		if (handle and dmgInfo:IsBulletDamage()) then
			...
			if (IsValid(client) and position:Distance(entity:GetBonePosition(handle)) <= 12) then
				...
				entity.ixOldSpeed = entity.ixOldSpeed or entity:GetKeyValues().speed or 100

				entity:Fire("setspeed", entity.ixOldSpeed * 3.5)
				entity:Fire("unlock")
				entity:Fire("openawayfrom", name)
				entity:EmitSound("physics/wood/wood_plank_break"..math.random(1, 4)..".wav", 100, 120)

				entity.ixNextBreach = CurTime() + 1

				timer.Simple(0.5, function()
					if (IsValid(entity)) then
						entity:Fire("setspeed", entity.ixOldSpeed)
					end
				end)
			end
		end
