...
function PLUGIN:IsEquippingGasmask(ply)
	...
	for x, items in pairs(inventory.slots) do
		for y, item in pairs(items) do
			if item.uniqueID == config.maskItem and item:GetData("equip") == true then
				return true
			elseif item.uniqueID == "hazmat_suit" and item:GetData("equip") == true then
				return true
			elseif item.uniqueID == "citizen_hazmat_suit" and item:GetData("equip") == true then
				return true
			end
		end
	end

	if (ply:Team() == FACTION_MPF or ply:Team() == FACTION_OTA) then
		return true
	end
